...
function MrrSparkSvg({ values }: Readonly<{ values: readonly number[] }>) {
  const w = 128;
  const h = 48;
  const padX = 2;
  const padY = 4;
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id="mrrSparkFill" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={mint} stopOpacity={0.38} />
          <stop offset="100%" stopColor={mint} stopOpacity={0.06} />
        </linearGradient>
      </defs>
      {areaD ? <path d={areaD} fill="url(#mrrSparkFill)" /> : null}
      {lineD ? (
        <path
          d={lineD}
          fill="none"
...
type MrrTrend = Readonly<{ label: string; tone: "up" | "down" | "flat" }>;

/** Stripe contract MRR + 30-day spark from daily snapshots. */
export function MrrPanel({
  ...
  const formatted = mrrCents !== null ? formatStripeCentsAsCurrency(mrrCents, currency) : "—";
  ...
  const showSpark = dailyMrrCents !== null && dailyMrrCents.length > 0;
    ...
    <div className="flex w-full min-w-0 flex-col gap-3 sm:flex-row sm:items-end sm:justify-between sm:gap-3 lg:gap-4">
      ...
      {showSpark ? (
        <div className="flex w-full min-w-0 shrink-0 justify-center sm:w-24 sm:justify-end sm:pl-1 md:w-28 lg:w-32 lg:pl-2">
          <MrrSparkSvg values={dailyMrrCents} />
        </div>
      ) : null}
    </div>
  );
